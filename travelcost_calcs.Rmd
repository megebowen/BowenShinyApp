...
choose_park_mo <- np_travel_costs %>% 
  ...
  filter(Month == "Mar") %>% 
  if_else(col == "Camp_Day",
          select(Camp_Day),
          select(-Camp_Day))

# filter by Transportation Selections
choose_house <- choose_park_mo %>% 
...
travel_transpo <- choose_transpo %>% 
      select(Month, Entrance_Fee, if_else(colnames = "Camp_Day", Camp_Day, Hotel_Day), ifelse("Fly_Trip", Fly_Trip, Car_Trip), Addnl_Boat_Fee)

TravelCost_Arches <- choose_transpo$Entrance_Fee + choose_transpo$Camp_Day + 2*(choose_transpo$Fly_Trip) + choose_transpo$Addnl_Fly_Fee + choose_transpo$Addnl_Boat_Fee 
